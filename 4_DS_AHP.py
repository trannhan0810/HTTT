...
def getSubSet(element, array):
    return [i for i in np.arange(len(array)) if all(x in element for x in array[i])]
...
def getRelateSet(element, array):
    return [i for i in np.arange(len(array)) if any(x in element for x in array[i])]
#Tính Pl với mass là giá trị ứng với tổ hợp comb tương ứng
#vd Pl[A] = mass[A] + mass[AB] + mass[AC] + mass[ABC]
def Pl(comb, mass):
   return np.array([np.sum([mass[getRelateSet(element, comb)]]) for element in comb])
...
def leftSideInequantion(combs, names):
    return np.array([[1 if(x in comb) else 0 for x in names] for comb in combs])
...
alternatives_Bel = np.array([Bel(alter_comb, alter_mass[key]) for key in range(num_of_cri) ])
alternatives_Pl = np.array([Pl(alter_comb, alter_mass[key]) for key in range(num_of_cri) ])

left_side, right_side = getCriteriaConditional(cri_comb, criterias, criteria_Pl, criteria_Bel)

c_bel = alternatives_Bel.transpose()
BEL = np.array([ np.dot((linprog(c, left_side, right_side).x), c) for c in c_bel])

c_pl = alternatives_Pl.transpose()
